...
def generate_handler(req):
    ...
    if not lesson_text and not prompt_override:
        return jsonify({"success": False, "error": "Missing input text for generation."}), 400
    ...
    if raw_result is None:
        return jsonify({"success": False, "error": "AI connection failed. Check API key or rate limits."}), 503
