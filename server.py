...
# ─── Router Table ────────────────────────────────────────────────────────────
ROUTES = {
    # (method, pattern) → handler function
...
class FRESHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, method):
        ...
        for (route_method, pattern), handler in ROUTES.items():
            ...
            if m:
                # Attach helpers so handlers can use them
...
if __name__ == "__main__":
    ...
    init_db()
    logger.info(f"🚀 FRES Backend running at http://127.0.0.1:{PORT}")
    server = HTTPServer(("127.0.0.1", PORT), FRESHandler)
